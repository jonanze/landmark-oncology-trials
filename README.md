...
`index.html` is generated from a curated workbook (`Landmark_Trials.xlsx`) by `build_dashboard.py`. The HTML is self-contained — all data is baked in, no server or network needed. Do not hand-edit `index.html`; change the workbook or the build script and regenerate.
